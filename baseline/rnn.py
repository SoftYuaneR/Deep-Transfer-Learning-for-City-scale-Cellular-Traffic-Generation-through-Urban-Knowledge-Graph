...
if kge_type == 'node':
    ...
    idx_use = np.load(os.path.join(transfer_generation_dir, ''))['idx_use']
    ...
    node_file = os.path.join(work_dir, 'node_embedding/node_embedding_KL_log_layerscat_d8_c2.npz')
    ...
    KGE_SIZE = 32
    type_dir = 'generated_data_1229_rnn_gan_original_kge'
    work_dir = ''
    save_dir_head = os.path.join(work_dir, type_dir)
    transfer_generation_dir = ''
    transfer_save_dir = os.path.join(transfer_generation_dir, type_dir)
   ...
   # idx_use = np.load(os.path.join(transfer_generation_dir, 'bs_record_nj_norm.npz'))['idx_use']
    ...
    node_file = os.path.join(work_dir, 'bs4graph_2000.npz')
...
if TRAIN:    
    ...
    for iteration in trange(ITERS):
        ...
        for idx, data in enumerate(data_loader):
           ...
           # D_real = netD(real_data,kge_batch,gpu)
            D_real = D_real.mean()
            # print D_real
