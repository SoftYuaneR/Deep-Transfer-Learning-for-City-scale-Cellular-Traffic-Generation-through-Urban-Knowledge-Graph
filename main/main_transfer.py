...
from tcgan import MultiGeneratorParallelRNN
from tcgan import MultiDiscriminatorParallel
...
def calc_gradient_penalty(netD, real_data, fake_data, kge):
    ...
    disc_interpolates, d_z, d_d, d_w, d_m = netD(interpolates.view(BATCH_SIZE,1,LENGTH), kge.view(BATCH_SIZE, -1, KGE_SIZE), gpu)
...
def generate_data(netG, kge, gene_size=128):
    ...
    noise_d = torch.randn(gene_size, NOISE_SIZE,  dim_list_g[0]-KGE_SIZE)
    noise_w = torch.randn(gene_size, NOISE_SIZE,  dim_list_g[0]-KGE_SIZE)
    noise_m = torch.randn(gene_size, NOISE_SIZE,  dim_list_g[0]-KGE_SIZE)
    if use_cuda:
        noise_d = noise_d.cuda(gpu)
        noise_w = noise_w.cuda(gpu)
        noise_m = noise_m.cuda(gpu)
        kge = kge.cuda(gpu)
    #noisev = autograd.Variable(noise, volatile=True)
    #noise_kge = torch.cat((noise, kge.view(kge.size(0),kge.size(1),1).expand(-1, -1, LENGTH)), 1)
    output, output_d, output_w, output_m, z_d, z_w, z_m = netG(torch.cat((noise_d, kge.view(gene_size, -1, KGE_SIZE)), 2),\
                                                               torch.cat((noise_w, kge.view(gene_size, -1, KGE_SIZE)), 2),\
                                                               torch.cat((noise_m, kge.view(gene_size, -1, KGE_SIZE)), 2))#, kge)
    
    #return output, kge, output_d, output_w, output_m, z
    return output, output_d, output_w, output_m, torch.cat((z_d, z_w, z_m), 1)
...
class TransferDataset(Dataset):
    def __init__(self, data, node_data, sample_rate=1):
        ...
        self.bs_record = torch.from_numpy(self.data['bs_record'][sample_list].astype(np.float32)).reshape(
        ...
        self.hours_in_weekday = torch.from_numpy(self.data['hours_in_weekday'][sample_list].astype(np.float32))
        self.hours_in_weekend = torch.from_numpy(self.data['hours_in_weekend'][sample_list].astype(np.float32))
        self.days_in_weekday = torch.from_numpy(self.data['days_in_weekday'][sample_list].astype(np.float32))
        self.days_in_weekend = torch.from_numpy(self.data['days_in_weekend'][sample_list].astype(np.float32))
        self.days_in_weekday_residual = torch.from_numpy(self.data['days_in_weekday_residual'][sample_list].astype(np.float32))
        self.days_in_weekend_residual = torch.from_numpy(self.data['days_in_weekend_residual'][sample_list].astype(np.float32))
...
LENGTH = 24*7#744
LAMBDA = 10
ITERS = 2#331 # How many iterations to train for
...
if kge_type == 'node':
   ...
   # idx_use = np.load(os.path.join(transfer_generation_dir, 'bs_record_w.npz'))
    transfer_node_file = os.path.join(transfer_generation_dir,
                                      '')
    transfer_node_data = np.load(transfer_node_file)['node_embedding']
    transfer_dataset = TransferDataset(data=os.path.join(transfer_generation_dir, ''),
    ...
    if not os.path.exists(transfer_save_dir):
        os.mkdir(transfer_save_dir)
    idx_use = np.load(os.path.join(transfer_generation_dir, 'bs_record_w.npz'))['idx_use']
    transfer_node_file = os.path.join(transfer_generation_dir,
                                      'bs4graph.npz')
    ...
    transfer_node_file = os.path.join(transfer_generation_dir,
                                      'bs4graph.npz')
    transfer_node_data = np.load(transfer_node_file)['poi_cate_dtb']
...
KGE_SQUEEZE_SIZE = 0#10
NOISE_SIZE = 1#0
input_size = KGE_SQUEEZE_SIZE + NOISE_SIZE
output_size = 1
layer_num = 6
num_channels = [1] * layer_num #[32, 16, 8, 4, 2, 1, 1]
kernel_size = [6, 24, 7*24]#[24, 7*24, 4*7*24]#[4*7, 4, 4*7*24]
dropout = 0.3
dim_list_g = [64+KGE_SIZE, 128, LENGTH]
dim_list_d = [LENGTH+KGE_SIZE, 128,  32, 1]
#dilation_size_list_G = []
#dilation_size_list_D = []
#for i in range(layer_num):
#    dilation_size_list_D.append(2 ** i)
#    dilation_size_list_G.append(2 ** (layer_num - i - 1))
dilation_size_list = ['', '', '']#[ [24]*layer_num, [7*24]*layer_num, '']#[4*7*24]*layer_num ]
netG = MultiGeneratorParallelRNN(dim_list_g, input_size, num_channels, kernel_size, dropout, dilation_size_list,kge_size=KGE_SIZE)#, KGE_SIZE, KGE_SQUEEZE_SIZE)
netD = MultiDiscriminatorParallel(dim_list_d, output_size, num_channels, kernel_size, dropout, dilation_size_list)#.double()

if LOAD_PRE_TRAIN:
    pretrained_netG = torch.load('./generated_data_0809_multi_kge_parallel_RNN/iteration-499/netG',map_location=torch.device('cpu'))#torch.load('./generated_data_0701_multi/iteration-199/netG',map_location=torch.device('cpu'))
    netG.load_state_dict(pretrained_netG)
    pretrained_netD = torch.load('./generated_data_0809_multi_kge_parallel_RNN/iteration-499/netD',map_location=torch.device('cpu'))
...
if GENE:
    save_dir = "./generated_data_0613_lg_gene"
...
if TRAIN:    
    ...
    for iteration in trange(ITERS):
        ...
        for idx, data in enumerate(data_loader):
            ...
            D_real, d_z, d_d, d_w, d_m = netD(real_data, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE), gpu)#, kge_batch)
            D_real = D_real.mean()
            # print D_real
            # TODO: Waiting for the bug fix from pytorch
            D_real.backward(mone)
            nn.utils.clip_grad_norm_(netD.parameters(), max_norm=3)
            
            # generate noise
            noise_batch_d = torch.randn(BATCH_SIZE, NOISE_SIZE,  dim_list_g[0]-KGE_SIZE)
            noise_batch_w = torch.randn(BATCH_SIZE, NOISE_SIZE,  dim_list_g[0]-KGE_SIZE)
            noise_batch_m = torch.randn(BATCH_SIZE, NOISE_SIZE,  dim_list_g[0]-KGE_SIZE)
            if use_cuda:
                noise_batch_d = noise_batch_d.cuda(gpu)
                noise_batch_w = noise_batch_w.cuda(gpu)
                noise_batch_m = noise_batch_m.cuda(gpu)				
            #noise_kge = torch.cat((noise_batch, kge_batch.view(kge_batch.size(0),kge_batch.size(1),1).expand(-1, -1, LENGTH)), 1)
            #noisev = autograd.Variable(noise, volatile=True)                
            # train with fake
            fake_data, fake_d, fake_w, fake_m, z_d, z_w, z_m = netG(torch.cat((noise_batch_d, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE)), 2),\
                                                        torch.cat((noise_batch_w, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE)), 2),\
														torch.cat((noise_batch_m, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE)), 2))#, kge_batch)
            #print('###############', iter_d, fake_data.shape)
            D_fake, d_z, d_d, d_w, d_m = netD(fake_data, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE), gpu)#, kge_batch)
            D_fake = D_fake.mean()
            # TODO: Waiting for the bug fix from pytorch
            D_fake.backward(one)
            ...
            if PRE_TRAIN & (iter_d%CRITIC_ITERS == 0):
                ############################
                # (2) Update G network
                ###########################
                ...
                #noise_kge = torch.cat((noise_batch, kge_batch.view(kge_batch.size(0),kge_batch.size(1),1).expand(-1, -1, LENGTH)), 1)
                fake, fake_d, fake_w, fake_m, z_d, z_w, z_m = netG(torch.cat((noise_batch_d, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE)), 2),\
                                                        torch.cat((noise_batch_w, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE)), 2),\
														torch.cat((noise_batch_m, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE)), 2))#, kge_batch)
                G, d_z, d_d, d_w, d_m = netD(fake, kge_batch.view(BATCH_SIZE, -1, KGE_SIZE), gpu)#, kge_batch)
                z_loss = torch.log(torch.cosh(z_d-d_d)).mean() \
                         + torch.log(torch.cosh(z_w-d_w)).mean() \
                         + torch.log(torch.cosh(z_m-d_m)).mean()
                z_loss.backward(one)
                ###z_loss_d = torch.log(torch.cosh(z_d-d_d)).mean()
                ###z_loss_d.backward(one)
                ###z_loss_w = torch.log(torch.cosh(z_w-d_w)).mean()
                ###z_loss_w.backward(one)
                ###z_loss_m = torch.log(torch.cosh(z_m-d_m)).mean()
                ###z_loss_m.backward(one)
				#G = G.mean()
                #G.backward(mone)
                #G_cost = -G
                nn.utils.clip_grad_norm_(netG.parameters(), max_norm=3)
                optimizerG.step()
        ...
        if iteration % 30 == 0:
            ...
            generated_data, generated_d, generated_w, generated_m, z = generate_data(netG, dataset.kge, gene_size)#[random.sample(range(DATASET_SIZE), gene_size)], gene_size)
        ...
        #if iteration % 10 == 0:
            print(G_cost, D_cost, Wasserstein_D, dst, z_loss)     
    #    except:
...
if TRANSFER_GENE:
    ...
    for seed in trange(GENE_NUM):
        ...
        generated_data, generated_d, generated_w, generated_m, z = generate_data(netG,
